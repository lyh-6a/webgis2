    ...
    <div class="maptools-view">
        ...
        <el-dropdown trigger="click" class="maptools-item" @command="handleCommand">
            ...
            <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item icon="el-icon-picture-outline" commands="printmap">地图打印</el-dropdown-item>
...
export default {
    ...
    methods: {
        ...
        async initDistanceMap() {
            const _self = this;
            const view = _self.$store.getters._getDefaultMapView;
            const [DistanceMeasurement2D] = await loadModules(['esri/widgets/DistanceMeasurement2D'], config.optionsoptions);
        ...
        async initAreaMap() {
            const _self = this;
            const view = _self.$store.getters._getDefaultMapView;
            const [AreaMeasurement2D] = await loadModules(['esri/widgets/AreaMeasurement2D'], config.optionsoptions);
            if (this.measurementWidget) this.measurementWidget.destroy();
 ...
 //自定义测量
        ...
        async initSpaceQuery() {
            const _self = this;
            const view = _self.$store.getters._getDefaultMapView;
            const [Graphic, GraphicsLayer] = await loadModules(['esri/Graphic', 'esri/layers/GraphicsLayer'], config.options);
            const resultLayer = view.map.findLayerById('polygonGraphicLayer');
            if (resultLayer) view.map.remove(resultLayer);
            _self.graphicsLayer = new GraphicsLayer({
                id: 'polygonGraphicLayer',
            ...
            self.sketchViewModel.on('create-complete', function (event) {
                const graphic = new Graphic({
                    geometry: event.geometry,
